---
id: state_cli-v2-config-commands
topic: bartender-cli
title: v2 指令設定化：per-指令 json＋SerializeReference 行為＋設定頁＋cli-cmd tag 分流
type: state
status: superseded
created_at: 2026-08-20
created_by: basecamp
links: [bartender-cli/state_cli-v1-shipped, bartender-cli/state_cli-v2-shipped-docs]
related_docs: []
---
...
## 未驗/未做
- 設定頁 UI 本體沒人開過（DrawObjectData 繪 List 的實際長相要 Tim 開頁看）；白名單成員實打 `cmd help` 走 config 路徑（Tim 是第二證人）。
- 文件未同步（Bartender 相關 md）；未 commit。
